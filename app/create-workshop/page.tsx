...
const availableServices = [
  "Inspection", "Repair", "Paint", "Tires", "AC Service",
  "Oil Change", "Detailing", "Body Work", "Engine Service", "Electrical",
];
...
export default function CreateWorkshopPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      setError(locale === "ar" ? "اسم الورشة مطلوب" : "Workshop name is required");
      return;
    }
    if (!city) {
      ...
      <div className="min-h-screen bg-slate-50 pt-24 pb-12">
        <div className="max-w-lg mx-auto px-4 text-center">
          ...
          <p className="text-slate-500 mb-8">
            {locale === "ar"
              ? "يجب عليك تسجيل الدخول لإنشاء ورشة"
              : "You need to sign in to create a workshop"}
          </p>
          <Link
            href="/auth/signin"
    ...
    return (
      <div className="min-h-screen bg-slate-50 pt-24 pb-12">
        <div className="max-w-lg mx-auto px-4 text-center">
          ...
          <p className="text-slate-500 mb-8">
            {locale === "ar"
              ? "فقط التجار والمسؤولين يمكنهم إنشاء ورش. تقدم لتصبح تاجراً أولاً."
              : "Only dealers and admins can create workshops. Apply to become a dealer first."}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              {locale === "ar" ? "تصفح الورش" : "Browse Workshops"}
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-slate-50 pt-24 pb-12">
        <div className="max-w-lg mx-auto px-4">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 text-center">
            ...
            <h1 className="text-2xl font-bold text-slate-900 mb-4">
              {locale === "ar" ? "تم إنشاء الورشة بنجاح!" : "Workshop Created!"}
            </h1>
            <p className="text-slate-500 mb-8">
              {locale === "ar"
                ? "ورشتك قيد المراجعة من قبل المسؤولين. ستظهر في صفحة الورش بعد الموافقة عليها."
                : "Your workshop is pending admin review. It will appear on the workshops page once approved."}
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                {locale === "ar" ? "عرض الورش" : "View Workshops"}
              </Link>
              <Link
                href="/sell"
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-slate-900 mb-2">
            {locale === "ar" ? "إنشاء ورشة جديدة" : "Create Workshop"}
          </h1>
          <p className="text-slate-500">
            {locale === "ar"
              ? "أضف ورشتك للوصول إلى المزيد من العملاء"
              : "Add your workshop to reach more customers"}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-100 p-6 sm:p-8 space-y-6">
          ...
          <div className="space-y-2">
            <label className={`block text-sm font-medium text-slate-700 ${dir === "rtl" ? "text-right" : ""}`}>
              <Store className="w-4 h-4 inline-block mb-1 me-2" />
              {locale === "ar" ? "اسم الورشة" : "Workshop Name"} <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder={locale === "ar" ? "مثال: ورشة الخليج للسيارات" : "e.g., Gulf Auto Service"}
          ...
          <div className="space-y-2">
            <label className={`block text-sm font-medium text-slate-700 ${dir === "rtl" ? "text-right" : ""}`}>
              <Wrench className="w-4 h-4 inline-block mb-1 me-2" />
            ...
            <p className={`text-xs text-slate-500 ${dir === "rtl" ? "text-right" : ""}`}>
              {locale === "ar" ? "اختر الخدمات التي تقدمها ورشتك" : "Select the services your workshop offers"}
            </p>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder={
                locale === "ar"
                  ? "أخبرنا المزيد عن ورشتك، خبرتك، وما يميزك..."
                  : "Tell us more about your workshop, experience, specialties..."
              }
              rows={4}
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                <span>{locale === "ar" ? "إنشاء الورشة" : "Create Workshop"}</span>
              </>
            )}
          </button>
        </form>
